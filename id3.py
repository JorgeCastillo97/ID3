...
conn = pymysql.connect('localhost','root', 'root', 'id3')
cursor = conn.cursor()
...
def main():
    ...
    """
    occurrences = getDecisionOccurrences("SELECT COUNT(*) FROM tennis WHERE decision = \"yes\"",
                                      "SELECT COUNT(*) FROM tennis WHERE decision = \"no\"",
                                       "SELECT COUNT(*) FROM tennis")

    print("Number of 'yes' and 'no' occurrences in the decision column:")
    print(occurrences)
    """ 
    We obtain the entropy using the following formula:
    ...
    """
    This dictionary contains the different values for any column of the tennis table
    cols = {"outlook":[sunny,overcast,rain], "temperature":[hot,mild,cool], "humidity":[high,normal],"wind":[weak,strong]}
    """
    column_values = getValues()
    #print("Different values for every column of the entire table:")
    #print(column_values)
    """
    We'll save the different values of gain in order to find the most determinant factor for decisioning
    """
    gains = getGain(column_values, entropy(occurrences["yes"], occurrences["no"], occurrences["rows"]))
    print("Factors on decision (Gain):")
    print(gains)
    rootNode = max(gains.values())
    rootNodeKey = None
    for key in gains:
        if gains[key] == rootNode:
            rootNodeKey = key
        else: key = None
    print("Root Node--> %s\tGain: %.3f" %(rootNodeKey, float(rootNode)))
    tree[rootNodeKey] = dict()
    for val in column_values[rootNodeKey]:
        #If outlook is overcast decision will always be yes
        if val == "overcast":
            tree[rootNodeKey][val] = "yes"
        else:
            tree[rootNodeKey][val] = None

    nodes = 0
    for key in tree[rootNodeKey]:
        if tree[rootNodeKey][key] == None:
    ...
    #print(tree)
    """
    We test for every element of the column outlook (sunny and rain)
    """
    i = 0;
    for i in range(len(column_values[rootNodeKey])):
        if not column_values[rootNodeKey][i] == "overcast":
            #print(column_values[rootNodeKey][i])
            node.__setitem__(getGainSubset(rootNodeKey, column_values[rootNodeKey][i], column_values), None)
            #print(node)
            #print("when %s check %s " %(column_values[rootNodeKey][i], node[column_values[rootNodeKey][i]]))
            tree[rootNodeKey][column_values[rootNodeKey][i]] = dict()
        else: pass
            #print("Decision will always be yes if outlook is overcast")


    #print(node)
    #for i in node:
        #print("For %s: %s"%(i,column_values[i]))

    cond = list(tree.keys())
    rootCondition = cond[0]
    #print(rootCondition)
    branchesConditions = list(node.keys())
    #print(branchesConditions[0])
    #print(column_values[rootCondition])
    brDecisions = getBrachesDecision(column_values[rootCondition], branchesConditions[0], branchesConditions[1],column_values)

    #Finaly we set the decision values of each branch in the decision tree
    for k in tree:
        for val in tree[k]:
            if val == 'sunny':
                tree[k][val] = brDecisions[0]
            elif val == 'rain':
                tree[k][val] = brDecisions[1]
            else:
                pass
...
def getValues():
    cols = {"outlook": None, "temperature": None, "humidity": None, "wind": None}
    i = 0
    for key in cols:
        cursor.execute("SELECT DISTINCT " + key + " FROM tennis")
...
def getGain(colVals, de):
    ...
    eOvercast = entropy(yesNoOccurences["outlook"][2], yesNoOccurences["outlook"][3], occurencesTotal["outlook"][1]) if not yesNoOccurences["outlook"].__contains__(0) else float(0)
    ...
    eHot = entropy(yesNoOccurences["temperature"][0], yesNoOccurences["temperature"][1], occurencesTotal["temperature"][0])
    colsEntropies.append(eHot)
    eMild = entropy(yesNoOccurences["temperature"][2], yesNoOccurences["temperature"][3], occurencesTotal["temperature"][1])
    colsEntropies.append(eMild)
    eCool = entropy(yesNoOccurences["temperature"][4], yesNoOccurences["temperature"][5], occurencesTotal["temperature"][2])
    ...
    humidityGain = de - ((occurencesTotal["humidity"][0])/14)*(eHigh) - ((occurencesTotal["humidity"][1])/14)*(eNormal)
    gain["humidity"] = format(humidityGain, ".3f")
    temperatureGain = de - ((occurencesTotal["temperature"][0])/14)*(eHot) - ((occurencesTotal["temperature"][1])/14)*(eMild) - ((occurencesTotal["temperature"][2])/14)*(eCool)
    gain["temperature"] = format(temperatureGain, ".3f")
    outlookGain = de - ((occurencesTotal["outlook"][0])/14)*(eSunny) - ((occurencesTotal["outlook"][1])/14)*(eOvercast) - ((occurencesTotal["outlook"][2])/14)*(eRain)
